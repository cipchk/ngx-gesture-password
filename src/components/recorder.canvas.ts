...
export class RecorderCanvas {
    ...
    render() {
        ...
        this.container.addEventListener('touchmove', (evt: any) => evt.preventDefault(), { passive: false });
        this.clearPath();
        return true;
    }
    ...
    async record(): Promise<Result> {
        ...
        let recordingTask: any = {};
        let promise = new Promise<Result>((resolve: any, reject: any) => {
            ...
            let done = (evt: any) => {
                ...
                let res = <Result>{ 
                    err, 
                    result: records.map(o => this.options.passwords[o.pos[0] * this.options.num + o.pos[1]]).join(''),
                    records
                };
    ...
    clearPath() {
        ...
        let range = Math.round(width / (num + 1));
